% a example code for generating 1D entangled chain
...
ham_cur1 = hamiltonian_1d_bhm2C(basis,ham_elems,J_up1,J_dn1,...
    U_uu,U_dd,U_ud,mu_up_lt,mu_dn_lt);
ham1 = ham_cur1.ham;
fprintf('Number of nonzero hamiltonian elements: %d.\n',nnz(ham1))

ham_cur2 = hamiltonian_1d_bhm2C(basis,ham_elems,J_up2,J_dn2,...
    U_uu,U_dd,U_ud,mu_up_lt,mu_dn_lt);
ham2 = ham_cur2.ham;
fprintf('Number of nonzero hamiltonian elements: %d.\n',nnz(ham2))
...
for kk = 2:nt
    ...
    tSC = tic;
    psic = expv(-1i*dt1,ham1,psic,1e-8,50);
    tEC = toc(tSC);
...
for kk = 2:nt
    ...
    tSC = tic;
    psic = expv(-1i*dt2,ham2,psic,1e-10,100);
    tEC = toc(tSC);
    fprintf('time for evolution: %.6f seconds.\n',tEC)
